...
static int mount_points_list_umount(MountPoint **head, bool *changed, bool last_try) {
        ...
        LIST_FOREACH(mount_point, m, *head) {
                ...
                if (r == -EPROTO && m->leaf) {
                        ...
                        if (r < 0) {
                                n_failed++;
                                log_full_errno(last_try ? LOG_ERR : LOG_INFO, errno, "Cannot find directory for %s: %m", m->path);
                                continue;
                        }

                        if (!path_equal_or_files_same(dirname, "/run/shutdown/mounts", 0)) {
                                _cleanup_free_ char *newpath = NULL, *hexhash = NULL;
                                uint8_t hash[SHA256_DIGEST_SIZE];

                                sha256_direct(m->path, strlen(m->path), hash);

                                hexhash = hexmem(hash, SHA256_DIGEST_SIZE);
                                if (!hexhash)
                                        return log_oom();

                                asprintf(&newpath, "/run/shutdown/mounts/%s", hexhash);
                                if (!newpath)
                                        return log_oom();
